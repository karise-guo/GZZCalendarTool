...
@implementation GZZCalendarTool
...
+ (NSString *)holiDayFromDate:(NSDate *)date {
    ...
    NSInteger weekOfMonth = [calendar component:NSCalendarUnitWeekOfMonth fromDate:date] - 1;
    NSInteger week = [calendar component:NSCalendarUnitWeekday fromDate:date] - 1;
    if (month == 5 && weekOfMonth == 2 && week == 0) {
        return @"母亲节"; // 5月第2个星期日
    }
    if (month == 6 && weekOfMonth == 3 && week == 0) {
        return @"父亲节"; // 6月第3个星期日
    }
    if (month == 11 && weekOfMonth == 4 && week == 4) {
        return @"感恩节"; // 11月第4个星期四
    }
    // 阳历节日
    NSDateFormatter *formatter = [[NSDateFormatter alloc] init];
    formatter.dateFormat = @"MM-dd";
    formatter.calendar = [[NSCalendar alloc] initWithCalendarIdentifier:NSCalendarIdentifierGregorian];
    NSDictionary *solarHolidays = @{ @"01-01": @"元旦节",
                                     @"02-14": @"情人节",
                                     @"03-05": @"雷锋日",
                                     @"03-08": @"妇女节",
                                     @"03-12": @"植树节",
...
+ (NSString *)solarTermFromDate:(NSDate *)date {
    ...
    int64_t flag = SOLAR_TERM_DAY[(component.year - START_YEAR) * 12 + component.month - 1];
    int64_t day = component.day < 15 ? (15 - ((flag>>4)&0x0f)) : (day = ((flag)&0x0f) + 15);
    NSInteger index = -1;
    if(component.day == day) {
        index = (component.month - 1) * 2 + (component.day > 15 ? 1: 0);
    }
    if (index >= 0  && index < solarTerms.count) {
        return [solarTerms objectAtIndex:index];
    }
    return nil;
}
